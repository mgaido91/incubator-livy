...
public class SessionManager extends CompositeService {
  ...
  private final Map<String, AtomicLong> connectionsCount = new ConcurrentHashMap<>();
  ...
  private void incrementConnections(final String username, final String ipAddress,
    ...
    if (violation == null) {
      if (trackConnectionsPerUser(username)) {
        incrementConnectionsCount(username);
      }

      if (trackConnectionsPerIpAddress(clientIpAddress)) {
        incrementConnectionsCount(clientIpAddress);
      }
  ...
  private void incrementConnectionsCount(final String key) {
    if (!connectionsCount.containsKey(key)) {
      connectionsCount.get(key).incrementAndGet();
    } else {
      connectionsCount.put(key, new AtomicLong());
    }
  }

  private void decrementConnectionsCount(final String key) {
    if (!connectionsCount.containsKey(key)) {
      connectionsCount.get(key).decrementAndGet();
    } else {
      connectionsCount.put(key, new AtomicLong());
    }
  }
