...
import javax.security.sasl.{AuthenticationException, AuthorizeCallback, Sasl}

import org.apache.hadoop.conf.Configuration
import org.apache.hadoop.fs.CommonConfigurationKeysPublic.HADOOP_SECURITY_AUTHENTICATION
import org.apache.hadoop.security.SaslRpcServer.AuthMethod
import org.apache.hive.service.auth.{PlainSaslHelper, SaslQOP, TSetIpAddressProcessor}
...
class AuthFactory(val conf: LivyConf) extends Logging {
  ...
  def getAuthTransFactory: TTransportFactory = {
    ...
    saslServer.map { server =>
      ...
      if (isAuthOther) {
        try {
          serverTransportFactory.addServerDefinition(
            "PLAIN",
            authTypeStr,
            null,
